...
public class Strings {
	...
	/**
	 * Flag to be used with {@link Strings#indexOf(String, int, String...)}<p> Using this flag will
	 * only return results outside of braces.
	 */
	 ...
	 * <code></code>state.getSource().substring(state.getSourceIndex)</code> the method does not
	 * copy any character or byte array.
	 *
	 ...
	 * non-whitespace characters remain unchanged, except the last line, where the trailing line
	 * break is also removed.
	 *
	...
	/**
	 * Capitalizes the given string, meaning the first character will be upper case, all following
	 * are lower case. If there are multiple words, still only the first character of the string
	 * will be capitalized!
	 *
	...
	/**
	 * Returns the enum constant referenced by the specified enum name. This method is very similar
	 * to T.value(name), except that it is case insensitive and provides the capability to specify a
	 * default value. The default value is used every time the specified name cannot be matched to a
	 * enum constant of the specified enum type. Therefore this method always returns a valid enum
	...
	/**
	 * Finds the ordinal or nth index of the given string in the given text. Note that the first
	 * occurrence is found by n = 0. Second occurrence by n = 1 and so on.
	 * <p>
	...
	/**
	 * Serializes a date object into a string using a standardized format. Can be un-serialized
	 * using {@link #readDate(String)}.
	 * <p>
